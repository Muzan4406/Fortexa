...
**How to apply:** Run `deploy-plesk.sh`, publish `artifacts/fortexa/public`, start `app.js` from the repository root, proxy `/api` to Node, and set Fortexa's own database and payment variables.